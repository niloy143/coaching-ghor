...
const Login = () => {
    ...
    const { dark, login, user, loading } = useContext(ContextCreator);
    ...
    return (
        user ? <Navigate to={state?.pathname || "/"} /> :
            <div>
                <form className={`${dark ? 'bg-slate-500 text-slate-100' : 'bg-slate-100 text-slate-700'} shadow-md w-11/12 sm:w-2/3 md:w-1/2 xl:w-1/3 mx-auto flex flex-col gap-2 p-5 mt-12 rounded-lg`} onSubmit={handleLogin}>
